...
const RecoveryWalletModal: React.FC<RecoveryWalletModalProps> = ({
  isOpen,
  onClose,
}) => {
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      >
        <ModalBody padding={0} display="flex" flexDirection="column">
          <RecoveryHeader
            step={ModalStepNumber[currentRecoveryStep]}
            onClose={onClose}
          />
          <VStack
            alignItems="center"
          ...
          >
            {currentRecoveryStep === ModalStep.VERIFY_2FA && (
              <RecoveryWallet2FAStep />
            )}
            {currentRecoveryStep === ModalStep.SECRET && (
              <RecoverWalletWithSecretStep
                setWebAuthURLForCreate={setWebAuthURLForCreate}
                setUserShare={setUserShare}
              />
            )}
            {currentRecoveryStep === ModalStep.LOST_2FA && (
              <RecoveryLost2FA onClose={onClose} />
            )}
            {currentRecoveryStep === ModalStep.BIOMETRIC && (
              <RecoveryBiometricsSetup url={webAuthURLForCreate} />
            )}
            {currentRecoveryStep === ModalStep.FINISH && (
              <RecoveryDoneStep onClose={onClose} />
            )}
            {currentRecoveryStep === ModalStep.AWAITING_FINISH && (
              <RecoveryAwaitingFinishStep />
            )}
          </VStack>
          <Footer />
        </ModalBody>
