...
export const SessionCosmosSign = () => {
  ...
  const handleReject = async (): Promise<void> => {
    if (!event || !event.topic) return;
    try {
      const response = await rejectRequest(event);
      await walletKit.respondSessionRequest({ topic: event.topic, response });
    } catch (error) {
      styledToast((error as Error).message, 'error');
    } finally {
  ...
  return (
    <>
      <Header metadata={session.peer.metadata} intention="sign a Cosmos transaction" verifyContext={event.verifyContext} />
      <SessionEventData />
      <SessionEventMethod />
