...
async function buildMobileProjects(): Promise<void> {
  const startTime = Date.now();
  console.log('🔨 Building mobile projects');
  console.log('============================================================');
  ...
  console.log('\n============================================================');
  console.log('📊 Build Summary (Mobile):');
  ...
  if (failed > 0) {
    ...
    console.log('🎉 All mobile builds completed successfully!');
  }
}

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log('Usage: tsx build-mobile.ts');
  console.log('');
  console.log('Builds all mobile projects that have build scripts');
  console.log('Automatically cleans build artifacts after successful builds');
  process.exit(0);
}

buildMobileProjects().catch(error => {
