...
const BUNDLE_CONFIG = {
  packages: {
    ...
    "@getpara/react-sdk": {
      ...
      entry: "dist/index.js",
      external: ["@getpara/cosmos-wallet-connectors", "@getpara/evm-wallet-connectors", "@getpara/solana-wallet-connectors", "@tanstack/react-query", "react", "react-dom", "@farcaster/miniapp-sdk"]
    },
    ...
    "@getpara/evm-wallet-connectors": {
      ...
      entry: "dist/index.js",
      external: ["@tanstack/react-query", "react", "react-dom", "viem", "wagmi", "@farcaster/miniapp-wagmi-connector", "@farcaster/miniapp-sdk"]
    },
    ...
    "@getpara/wagmi-v2-integration": {
      ...
      entry: "dist/index.js",
      external: ["@getpara/cosmos-wallet-connectors", "@getpara/evm-wallet-connectors", "@getpara/solana-wallet-connectors", "@tanstack/react-query", "react", "react-dom", "viem", "wagmi", "@farcaster/miniapp-sdk"]
    },
  ...
  for (const packageInfo of packages) {
    ...
    for (const peerDep of allPeerDeps) {
      if (!configExternals.has(peerDep)) {
        errors.push(`${packageInfo.packageJson.name}: peer dependency "${peerDep}" (direct or transitive) not listed as external`);
      }
    }
    ...
    for (const external of configExternals) {
      if (!allPeerDeps.has(external)) {
        errors.push(`${packageInfo.packageJson.name}: external dependency "${external}" not found as direct or transitive peer dependency`);
      }
    }
  }
