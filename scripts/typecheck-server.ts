...
async function typecheckServerProjects(): Promise<void> {
  const startTime = Date.now();
  console.log('📝 Running typecheck in server projects');
  console.log('============================================================');
  ...
  console.log('\n============================================================');
  console.log('📊 Typecheck Summary (Server):');
  ...
  if (failed > 0) {
    ...
    console.log('🎉 All server typecheck passed!');
  }
}

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log('Usage: tsx typecheck-server.ts');
  console.log('');
  console.log('Runs typecheck on all server projects that have typecheck scripts');
  process.exit(0);
}

typecheckServerProjects().catch(error => {
