...
async function buildWebProjects(): Promise<void> {
  const startTime = Date.now();
  console.log('🔨 Building web projects');
  console.log('============================================================');
  ...
  if (failed > 0) {
    ...
    console.log('🎉 All web builds completed successfully!');
  }
}

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log('Usage: tsx build-web.ts');
  console.log('');
  console.log('Builds all web projects that have build scripts');
  console.log('Automatically cleans build artifacts after successful builds');
  process.exit(0);
}

buildWebProjects().catch(error => {
