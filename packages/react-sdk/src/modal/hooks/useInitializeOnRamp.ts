...
export function useInitializeOnRamp(provider: OnRampConfigProvider, assetProp: OnRampAssetProp, testMode = false) {
  ...
  return async () => {
    const newOnRampPurchase = await capsule.createOnRampPurchase(provider.id, assetProp, testMode);
    setOnRampPurchase(newOnRampPurchase);

    const wallet = capsule.getWallets()[newOnRampPurchase.walletId];
    ...
    if (isRamp(provider)) {
      ...
      const widget = new RampInstantSDK({
        ...
        .on('PURCHASE_CREATED' as unknown as '*', async (e) => {
          ...
          const updated = await capsule.updateOnRampPurchase(wallet.id, newOnRampPurchase.id, {
            providerKey: p.id,
