...
const Wallet = ({ wallet, withCopy, slot }: { wallet: Omit<TWallet, 'signer'>; withCopy?: boolean; slot?: string }) => {
...
export const WalletSelect = () => {
  ...
  const ActiveWalletNode = activeWallet ? <Wallet withCopy wallet={activeWallet} slot="selected-item" /> : null;
          ...
          {(availableWallets || []).map(({ address, name: _name, id, type }) => {
            ...
            return (
              <SelectItem key={key} slot="items" value={key}>
                <Wallet wallet={{ address, name: _name, id, type }} />
              </SelectItem>
            );
          })}
        </Select>
