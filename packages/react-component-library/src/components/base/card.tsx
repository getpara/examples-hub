...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'para:bg-card para:text-card-foreground para:flex para:flex-col para:gap-6 para:rounded-xl para:border para:py-6 para:shadow-sm',
        className,
      )}
      {...props}
    />
...
function CardHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div data-slot="card-header" className={cn('para:flex para:flex-col para:gap-1.5 para:px-6', className)} {...props} />
  );
}
