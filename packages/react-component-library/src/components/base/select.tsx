...
function SelectTrigger({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger>) {
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "para:border-border para:data-[placeholder]:text-muted-foreground para:[&_svg:not([class*='text-'])]:text-muted-foreground para:focus-visible:border-ring para:focus-visible:ring-ring/50 para:aria-invalid:ring-destructive/20 para:dark:aria-invalid:ring-destructive/40 para:aria-invalid:border-destructive para:flex para:h-12 para:w-fit para:items-center para:justify-between para:gap-2 para:rounded para:border para:bg-muted para:px-3 para:py-2 para:text-sm para:whitespace-nowrap para:transition-[color,box-shadow] para:outline-none para:focus-visible:ring-[3px] para:disabled:cursor-not-allowed para:disabled:opacity-50 para:*:data-[slot=select-value]:line-clamp-1 para:*:data-[slot=select-value]:flex para:*:data-[slot=select-value]:items-center para:*:data-[slot=select-value]:gap-2 para:[&_svg]:pointer-events-none para:[&_svg]:shrink-0 para:[&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
