...
export const signWithZerodevEIP7702 = async (req: Request): Promise<Response> => {
  try {
    const body = await req.json();
    const session = body.session as string | undefined;

    if (!session) {
      return Response.json({ success: false, message: "Provide session in the request body" }, { status: 400 });
    }
