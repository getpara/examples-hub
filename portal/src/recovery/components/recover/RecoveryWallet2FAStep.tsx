...
import Console from '../../../library/modal/assets/console';
...
const RecoveryWallet2FAStep: React.FC = () => {
    ...
    return (
        <VStack flex={1}>
            ...
            <Button
                ...
                onClick={async () => {
                    if (verificationCode.length === 6 && /^\d+$/.test(verificationCode)) {
                        try {
                            const { walletId, userId } = await capsule.verify2FA(email, verificationCode);
            ...
            <Button
                variant="link"
                onClick={() => setCurrentRecoveryStep(ModalStep.LOST_2FA)}
            >
