...
export function credsToParaAuth(
  c: AuthCreds
): { email: string } | { phone: string; countryCode: string } {
  return c.authType === 'email'
    ? { email: c.email }
    : { phone: c.phone, countryCode: c.countryCode };
}
...
export function buildPasskeyArgs(
  ...
  | { biometricsId: string; email: string }
  | { biometricsId: string; phone: string; countryCode: string } {
