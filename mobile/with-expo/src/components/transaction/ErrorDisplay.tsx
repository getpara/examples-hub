...
export function ErrorDisplay({
  ...
  const getErrorIcon = () => {
    switch (error.category) {
      ...
      default:
        return <XCircle size={24} className="text-destructive" />;
    }
  };
    ...
    <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <CardContent className="p-4">
          ...
          {txHash && (
            <View className="py-3 border-t border-border">
              ...
              <Text className="text-sm text-muted-foreground break-all">
                {txHash}
              </Text>
            </View>
          )}
          ...
          <View className="space-y-2">
            {getRemediationSteps().map((step, index) => (
              <View key={index} className="flex-row items-start">
                <AlertCircle size={16} className="text-primary mt-0.5 mr-2" />
      ...
      <View className="flex-row gap-x-4 mb-10">
        {onRetry && (
          ...
          >
            <RefreshCcw size={18} className="text-primary-foreground mr-2" />
