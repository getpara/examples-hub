...
export function TransactionDetails({
  ...
  const renderDetailRow = (label: string, value: React.ReactNode) => (
    <View className="flex-row justify-between py-2">
      ...
      {typeof value === 'string' ? (
        <Text className="text-sm text-right text-foreground">{value}</Text>
      ) : (
        value
      )}
    </View>
  ...
  const renderSolanaDetails = () => (
    <View>
      {computeUnits &&
        renderDetailRow('Compute Budget', computeUnits.toString())}
      {computeUnitsUsed &&
        renderDetailRow('Compute Units Used', computeUnitsUsed.toString())}
      ...
      <Card className="border border-border mb-4">
        <CardContent className="p-4">
          ...
          {elapsedTime !== undefined &&
            renderDetailRow('Elapsed Time', formatDuration(elapsedTime))}
