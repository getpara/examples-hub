...
export function StatusIndicator({ status, networkType, elapsedTime = 0, estimatedTime }: StatusIndicatorProps) {
  ...
  const renderStatusContent = () => {
    switch (status) {
      case "pending":
        return (
          <View className="items-center">
            <Text className="text-xl font-bold text-foreground mt-4 mb-2">Transaction Pending</Text>
            ...
            <Text className="text-sm text-muted-foreground text-center">Elapsed time: {formatTime(elapsedTime)}</Text>
      ...
      case "confirmed":
        return (
          <View className="items-center">
            <Text className="text-xl font-bold text-foreground mt-4 mb-2">Transaction Confirmed</Text>
            ...
            <View className="flex-row items-center mt-4 bg-green-100 rounded-full px-4 py-2">
              ...
              />
              <Text className="text-sm text-green-600">Completed in {formatTime(elapsedTime)}</Text>
            </View>
          </View>
        );
      case "failed":
        return (
          <View className="items-center">
            <Text className="text-xl font-bold text-foreground mt-4 mb-2">Transaction Failed</Text>
            <Text className="text-base text-muted-foreground text-center">
              Your transaction on the {networkType === WalletType.EVM ? "Ethereum" : "Solana"} network could not be
              completed
            </Text>
            <View className="flex-row items-center mt-4 bg-destructive/10 rounded-full px-4 py-2">
              ...
              />
              <Text className="text-sm text-destructive">Failed after {formatTime(elapsedTime)}</Text>
            </View>
          </View>
        );
      default:
