...
import { ArrowUpFromLine, ArrowDownToLine, CheckCircle, Clock, XCircle } from "@/components/icons";
...
export function TransactionListItem({ transaction, onPress }: TransactionItemProps) {
  const { type, status, timestamp, amount, tokenTicker, amountUsd, counterpartyAddress, counterpartyName } =
    transaction;
    ...
    <Pressable
      ...
      <View className="flex-1 ml-3">
        <View className="flex-row items-center justify-between">
          ...
          <Text className={`text-base font-medium ${type === "send" ? "text-red-600" : "text-green-600"}`}>
            {type === "send" ? "-" : "+"}
            {amount} {tokenTicker}
          </Text>
        </View>
        ...
        <View className="flex-row items-center justify-between mt-1">
          <View>
            <Text className="text-xs text-muted-foreground">{formatDate(timestamp)}</Text>
            <View className="flex-row items-center mt-1">
              <Text className="text-xs text-muted-foreground mr-2">
                {type === "send" ? "To:" : "From:"} {formatAddress(counterpartyAddress, transaction.networkType)}
              </Text>
              {getStatusIndicator()}
            </View>
          </View>

          {amountUsd !== null && <Text className="text-xs text-muted-foreground">{formatUsdValue(amountUsd)}</Text>}
        </View>

        {networkDetail ? <Text className="text-xs text-muted-foreground mt-1">{networkDetail}</Text> : null}
      </View>
    </Pressable>
  );
}
