...
export function TokenList({
  ...
  const renderTokenGroup = (
    ...
    return (
      <View className="mb-6" key={networkType}>
          ...
          {tokens.map((token) => (
            <TokenItem
              ...
              logo={
                token.logoUri
                  ? { uri: token.logoUri }
                  : getTokenLogo(token.ticker)
              }
              networkType={networkType as SupportedWalletType}
              networkLogo={networkIcon || require('~/assets/para.png')}
  ...
  return (
    <View className="flex-1">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) =>
          onTabChange(value as 'all' | SupportedWalletType)
        }
        className="w-full mb-4"
      >
        <TabsList className="w-full h-auto bg-card border border-border p-1 rounded-lg">
          <TabsTrigger value="all" className="flex-1 rounded-md py-2">
            <Text>All</Text>
          </TabsTrigger>
          <TabsTrigger
      ...
      {isLoading ? (
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          {activeTab === 'all' ? (
            <>
              {SUPPORTED_WALLET_TYPES.map((type) =>
                renderTokenGroup(type, groupedTokens[type])
              )}
            </>
          ) : (
