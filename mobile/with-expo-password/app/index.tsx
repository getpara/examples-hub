...
export default function AuthScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = para.onStatePhaseChange(async (snapshot: StateSnapshot) => {
      ...
      if (
        ...
        authStateInfo.passwordUrl !== lastUrlRef.current
      ) {
        lastUrlRef.current = authStateInfo.passwordUrl;
        setAuthState('awaiting_browser');
        const result = await openAuthUrl(authStateInfo.passwordUrl);
        if (result.success) {
  ...
  async function handleVerify() {
    ...
    try {
      setAuthState('authenticating');
      await verifyNewAccountAsync({ verificationCode: otpCode.trim() });
    } catch (err) {
  ...
  async function handleResendCode() {
    try {
      ...
      // Silently handle resend failures
    }
  }
        ...
        >
          ...
          <View style={styles.bottom}>
            ...
            {authState === 'awaiting_browser' && (
              <LoadingView message="Complete password setup in the browser" />
            )}

            {authState === 'error' && (
              <ErrorView
                message={errorMessage}
                onRetry={() => { setAuthState('idle'); setErrorMessage(''); setOtpCode(''); lastUrlRef.current = null; }}
              />
            )}
          </View>
        </KeyboardAvoidingView>
