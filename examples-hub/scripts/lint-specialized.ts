...
async function lintSpecializedProjects(): Promise<void> {
  const startTime = Date.now();
  console.log('🔍 Running lint in specialized projects');
  console.log('============================================================');
  ...
  console.log('\n============================================================');
  console.log('📊 Lint Summary (Specialized):');
  ...
  if (failed > 0) {
    ...
    console.log('🎉 All specialized lint checks passed!');
  }
}

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log('Usage: tsx lint-specialized.ts');
  console.log('');
  console.log('Runs lint on all specialized projects that have lint scripts');
  process.exit(0);
}

lintSpecializedProjects().catch(error => {
