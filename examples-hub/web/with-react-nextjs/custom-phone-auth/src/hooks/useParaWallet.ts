...
export function useParaWallet() {
  ...
  const createWalletMutation = useMutation({
    mutationFn: async ({ skipDistribute = false }: CreateWalletParams) => {
      return await para.createWallet({
        type: "EVM",
        skipDistribute,
      });
    },
    onSuccess: () => {
      // Invalidate wallet queries after wallet creation
  ...
  const waitForWalletCreationMutation = useMutation({
    mutationFn: async ({ isCanceled }: WaitForWalletCreationParams) => {
      return await para.waitForWalletCreation({ isCanceled });
    },
    onSuccess: () => {
      // Invalidate account queries after wallet creation
