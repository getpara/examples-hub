  ...
  const deployContract = async () => {
    ...
    try {
      ...
      setStatus({
        ...
        type: "info",
        message:
          "Contract deployment transaction submitted. Waiting for confirmation...",
      });

      const deployedContract = await contractDeployment.waitForDeployment();
      const deployTx = contractDeployment.deploymentTransaction();
    ...
    <div className="container mx-auto px-4">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight mb-6">
          Contract Deployment Demo
        </h1>
        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
          Deploy an ERC20 token contract using the Para SDK with ethers.js v6
          integration. This demo uses the{" "}
              ...
              <div className="p-6">
                <p className="text-sm font-mono break-all text-gray-600 bg-white p-4 border border-gray-200">
                  {deploymentInfo.deployedBytecode
                    ? `${deploymentInfo.deployedBytecode.slice(
                        0,
                        128
                      )}...${deploymentInfo.deployedBytecode.slice(-128)}`
                    : ""}
                </p>
              </div>
            </div>
