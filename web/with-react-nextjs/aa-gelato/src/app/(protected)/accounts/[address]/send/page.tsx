...
import { getGelatoClient } from "@/lib/create-gelato-client";
...
import type { GelatoTransactionStatus, GelatoTransactionError } from "@/types/gelato";
...
export default function SendAssetsPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const walletIndex = 0;
      const { client } = await getGelatoClient(para, BigInt(walletIndex));
      ...
      const txHash = await new Promise<string>((resolve, reject) => {
        response.on("success", (status: GelatoTransactionStatus) => {
        ...
        response.on("error", (error: GelatoTransactionError) => {
          reject(error);
        });
      });
