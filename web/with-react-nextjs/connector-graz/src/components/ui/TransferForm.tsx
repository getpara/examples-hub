interface TransferFormProps {
  to: string;
  amount: string;
  isLoading: boolean;
  denom?: string;
  onToChange: (value: string) => void;
  onAmountChange: (value: string) => void;
  onSubmit: (e: React.FormEvent) => void;
}

export function TransferForm({ amount, isLoading, denom = "ATOM", onAmountChange, onSubmit }: TransferFormProps) {
  const faucetAddress = "cosmos1qdvzqujxqd0pqwcdtpxgfcqcvxn777ka3xmn4u";

  return (
    <form
      onSubmit={onSubmit}
      data-testid="transfer-form"
      className="rounded-none border border-gray-200">
      <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Return Tokens to Faucet</h3>
      </div>
      <div className="px-6 py-4 space-y-4">
        <div>
          <label
            htmlFor="to"
            className="block text-sm font-medium text-gray-700 mb-1">
            To Address (Faucet)
          </label>
          <input
            id="to"
            type="text"
            data-testid="tx-to-input"
            value={faucetAddress}
            disabled
            className="w-full px-3 py-2 rounded-none border border-gray-300 bg-gray-100 text-gray-600"
          />
        </div>
        <div>
          <label
            htmlFor="amount"
            className="block text-sm font-medium text-gray-700 mb-1">
            Amount ({denom})
          </label>
          <input
            id="amount"
            type="text"
            data-testid="tx-amount-input"
            value={amount}
            onChange={(e) => onAmountChange(e.target.value)}
            placeholder="0.001"
            className="w-full px-3 py-2 rounded-none border border-gray-300 focus:border-gray-400 focus:outline-none"
            required
          />
        </div>
        <button
          type="submit"
          data-testid="tx-submit-button"
          disabled={isLoading}
          className="w-full rounded-none bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-950 disabled:bg-gray-400 transition-colors cursor-pointer">
          {isLoading ? "Sending..." : "Send Transaction"}
        </button>
      </div>
    </form>
  );
}
