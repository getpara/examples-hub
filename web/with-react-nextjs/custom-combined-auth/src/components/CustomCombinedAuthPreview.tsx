...
export function CustomCombinedAuthPreview() {
  return (
      ...
      <section className="mx-auto grid max-w-5xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(22rem,1fr)] lg:px-8 lg:py-16">
        ...
        <div className="animate-fade-in-up w-full overflow-hidden rounded-lg border border-border bg-card shadow-sm">
          <div className="border-b border-border/60 px-6 py-5">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Choose an auth method backed by Para.</p>
          </div>
          <div className="space-y-5 px-6 py-5">
            <div className="grid grid-cols-3 gap-1 rounded-lg bg-muted p-1">
              {["Email", "Phone", "Social"].map((label) => (
                <button
                  type="button"
                  key={label}
                  disabled
                  className={`min-h-10 rounded-md px-3 text-sm font-medium ${
                    label === "Email" ? "bg-card text-card-foreground shadow-sm" : "text-muted-foreground"
                  }`}>
                  {label}
                </button>
              ))}
