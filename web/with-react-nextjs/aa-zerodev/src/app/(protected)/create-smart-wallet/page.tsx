...
import { useNextAvailableWalletIndex } from "@/hooks/useNextAvailableWalletIndex";
import { useSmartWallets } from "@/hooks/useSmartWallets";
import { useDeploymentFee } from "@/hooks/useDeploymentFee";
import { MAX_SMART_WALLETS_PER_EOA, WALLET_LIMIT_MESSAGE } from "@/config/smart-wallet";
...
export default function CreateAccountPage() {
  ...
  const { address: predictedAddress, isLoading: isAddressLoading } = useSmartWalletAddress(nextIndex ?? 0);

  const { mutate: deployWallet, isPending: isDeploying } = useDeploySmartWallet();

  const { fee: deploymentFee, isLoading: isFeeLoading } = useDeploymentFee(wallet?.id || null, nextIndex ?? 0);
      ...
      <div className="container mx-auto py-8 px-4 flex-1 flex flex-col items-center justify-center z-0">
        <Card className="w-full max-w-lg max-h-[85vh] overflow-auto text-center">
          <CardHeader className="pt-8">
            ...
            <CardDescription>
              You have reached the maximum of {MAX_SMART_WALLETS_PER_EOA} smart wallets. To create a new one,
              you&apos;ll need to use a different EOA.
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button
