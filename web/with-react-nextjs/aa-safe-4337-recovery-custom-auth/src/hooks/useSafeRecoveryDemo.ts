...
interface UseSafeRecoveryDemoOptions {
  para: ParaWeb | null;
  guardianWalletAddress: `0x${string}` | null;
  guardianWalletId: string | null;
  enabled?: boolean;
}

export function useSafeRecoveryDemo({
  para,
  guardianWalletAddress,
  guardianWalletId,
  enabled = true,
}: UseSafeRecoveryDemoOptions) {
  ...
  const guardianAccount = useMemo(() => {
    if (!para || !guardianWalletAddress) return null;
    return createParaViemAccount({ para, address: guardianWalletAddress });
  }, [guardianWalletAddress, para]);
  ...
  const protectSafe = useCallback(async () => {
    await runAction("Enabling recovery module", async () => {
      ...
      if (!safe || !state.safeAddress || !guardianAccount?.address) {
  ...
  }, [guardianWalletId, para, runAction]);
  ...
  }, [guardianAccount, runAction, state.newOwnerAddress, state.safeAddress]);
  ...
  return useMemo(
    () => ({
      ...
      guardianFundingMessage: state.protectUserOpHash && !state.faucetTxHash && !state.recoveryTxHash
        ...
        : null,
      isFaucetPending,
      isGuardianAccountLoading: false,
      canCreateSafe: enabled && !state.isWorking,
      canProtectSafe: enabled && !state.isWorking && Boolean(state.safeAddress && state.createTxHash && guardianAccount?.address),
      canRequestFunds: enabled && !state.isWorking && Boolean(guardianWalletId) && !isFaucetPending,
      canSendNormalTransaction: enabled && !state.isWorking && Boolean(state.protectUserOpHash),
      canProveGuardianCannotSpend: enabled && !state.isWorking && Boolean(state.protectUserOpHash && guardianAccount?.address),
