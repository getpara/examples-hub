...
export default function SolTransferPage() {
  ...
  const constructTransaction = async (toAddress: string, solAmount: string): Promise<Transaction> => {
    if (!address || !connection || !anchorProvider) throw new Error("No sender address or connection available");
    ...
    try {
      ...
      if (!anchorProvider) {
        setStatus({
          show: true,
          type: "error",
          message: "No anchor provider found. Please reconnect your wallet.",
        });
        return;
      }
